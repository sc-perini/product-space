...
Este código se extrajo de <https://github.com/ejokeeffe/tradedownloader/tree/master/tradedownloader>
...
Se realizaron modificaciones/actualizaciones por estar deprecadas algunas funciones. 

@date updates 14/01/2021
...
class ComtradeApi:
    _source_folder = ""
    _url = 'https://comtrade.un.org/api/get?'
    _ctry_codes = []
    _ctry_alt_names = []
    _max_partners = 5
    _max_years = 5
    ...
    def __init__(self, ctry_codes_path="UN Comtrade Country List.csv", fld=""):
        ...
        if (os.path.isfile(saved_queries_fname)):
            ...
            self._saved_queries = pd.DataFrame({'id': [], 'querystring': [],
                                                'comcode': [], 'year': [], 'freq': []})
        self._saved_queries.comcode = self._saved_queries.comcode.astype(np.str)
        self._saved_queries.freq = self._saved_queries.freq.astype(np.str) # cambio pd.np.str por np.str directo porque deprecated
        self._saved_queries.querystring = self._saved_queries.querystring.astype(np.str)
                    ...
                    for com in comcodes:
                        ...
                        if (fname > -1):
                            ...
                            logging.info("{} for year {} and freq {} not available in comtrade. Fname: {}".format(
                                com, yr, freq, fname))
            load_files = np.unique(load_files)
